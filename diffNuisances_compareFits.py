...
parser.add_option("-o", "--outputPDF",   dest="outputPDF", default="ttH_hbb_13TeV", type="string",  help="Output pdf name for nuisance pulls")
...
file_comb = ROOT.TFile(args[0])
file_sl = ROOT.TFile(args[1])
file_dl = ROOT.TFile(args[2])
if file_comb == None: raise RuntimeError, "Cannot open file %s" % args[0]
fit_comb_s  = file_comb.Get("fit_s")
fit_sl_s  = file_sl.Get("fit_s")
fit_dl_s  = file_dl.Get("fit_s")
prefit = file_comb.Get("nuisances_prefit")
...
fpf_comb_s = fit_comb_s.floatParsFinal()
fpf_sl_s = fit_sl_s.floatParsFinal()
fpf_dl_s = fit_dl_s.floatParsFinal()
...
nNuis_noMCstat = 0;
for i in range(fpf_comb_s.getSize()):
    nuis = fpf_comb_s.at(i)
    name = nuis.GetName()
    if "BDTbin" in name:
        continue
    if name=="r":
...
hist_fit_comb_s  = ROOT.TH1F("postfit_fit_comb_s"   ,"S+B fit Nuisances   ;;#theta ",nNuis_noMCstat,0,nNuis_noMCstat)
hist_fit_sl_s  = ROOT.TH1F("postfit_fit_sl_s"   ,"S+B fit Nuisances   ;;#theta ",nNuis_noMCstat,0,nNuis_noMCstat)
hist_fit_dl_s  = ROOT.TH1F("postfit_fit_dl_s"   ,"S+B fit Nuisances   ;;#theta ",nNuis_noMCstat,0,nNuis_noMCstat)
hist_prefit = ROOT.TH1F("prefit_nuisancs","Prefit Nuisances    ;;#theta ",nNuis_noMCstat,0,nNuis_noMCstat)

# loop over all fitted parameters
for i in range(fpf_comb_s.getSize()):

    nuis_comb_s = fpf_comb_s.at(i)
    name   = nuis_comb_s.GetName();
    nuis_comb_s = fpf_comb_s.find(name)
    ...
    if nuis_p == None:
        ...
        row += [ "[%.2f, %.2f]" % (nuis_comb_s.getMin(), nuis_comb_s.getMax()) ]
    ...
    for fit_name, nuis_x in [('s', nuis_comb_s)]:
        if nuis_x == None:
            row += [ " n/a " ]
        else:
            row += [ "%+.2f +/- %.2f" % (nuis_x.getVal(), nuis_x.getError()) ]

            nuis_sl_x = fpf_sl_s.find(name)
            if nuis_sl_x == None:
                row += [ " n/a " ]
            else :
                row += [ "%+.2f +/- %.2f" % (nuis_sl_x.getVal(), nuis_sl_x.getError()) ]

            nuis_dl_x = fpf_dl_s.find(name)
            if nuis_dl_x == None:
                row += [ " n/a " ]
            else :
                row += [ "%+.2f +/- %.2f" % (nuis_dl_x.getVal(), nuis_dl_x.getError()) ]
      
            if nuis_p != None:
                if options.plotfile:
                    nuis_p_i+=1
                    hist_fit_comb_s.SetBinContent(nuis_p_i,nuis_x.getVal())
                    hist_fit_comb_s.SetBinError(nuis_p_i,nuis_x.getError())
                    hist_fit_comb_s.GetXaxis().SetBinLabel(nuis_p_i,name)
                    if nuis_sl_x != None:
                        hist_fit_sl_s.SetBinContent(nuis_p_i,nuis_sl_x.getVal())
                        hist_fit_sl_s.SetBinError(nuis_p_i,nuis_sl_x.getError())
                        hist_fit_sl_s.GetXaxis().SetBinLabel(nuis_p_i,name)
                    if nuis_dl_x != None:
                        hist_fit_dl_s.SetBinContent(nuis_p_i,nuis_dl_x.getVal())
                        hist_fit_dl_s.SetBinError(nuis_p_i,nuis_dl_x.getError())
                        hist_fit_dl_s.GetXaxis().SetBinLabel(nuis_p_i,name)
    ...
    row += [ "%+4.2f"  % fit_comb_s.correlation(name, options.poi) ]
...
if options.format == 'text':
    if options.abs:
        ...
        print fmtstring % ('name', 'combined s+b fit', ' SL s+b fit', ' DL s+b fit', 'rho')
...
if options.plotfile:
    ...
    canvas_nuis = ROOT.TCanvas("nuisances", "nuisances", 1200, 600)
    hist_fit_comb_s = hist_fit_comb_s.Clone("errors_comb_s")
    hist_fit_sl_s = hist_fit_sl_s.Clone("errors_sl_s")
    hist_fit_dl_s = hist_fit_dl_s.Clone("errors_dl_s")
    gr_fit_comb_s = getGraph(hist_fit_comb_s,-0.2)
    gr_fit_sl_s = getGraph(hist_fit_sl_s,0.0)
    gr_fit_dl_s = getGraph(hist_fit_dl_s,0.2)
    gr_fit_comb_s.SetLineColor(ROOT.kGreen+1)
    gr_fit_comb_s.SetMarkerColor(ROOT.kGreen+1)
    gr_fit_sl_s.SetLineColor(ROOT.kBlue)
    gr_fit_sl_s.SetMarkerColor(ROOT.kBlue)
    gr_fit_dl_s.SetLineColor(ROOT.kRed)
    gr_fit_dl_s.SetMarkerColor(ROOT.kRed)
    gr_fit_comb_s.SetMarkerStyle(20)
    gr_fit_sl_s.SetMarkerStyle(20)
    gr_fit_dl_s.SetMarkerStyle(20)
    gr_fit_comb_s.SetMarkerSize(1.0)
    gr_fit_sl_s.SetMarkerSize(1.0)
    gr_fit_dl_s.SetMarkerSize(1.0)
    gr_fit_comb_s.SetLineWidth(2)
    gr_fit_sl_s.SetLineWidth(2)
    gr_fit_dl_s.SetLineWidth(2)
    ...
    gr_fit_comb_s.Draw("EPsame")
    gr_fit_sl_s.Draw("EPsame")
    gr_fit_dl_s.Draw("EPsame")
    canvas_nuis.SetRightMargin(0.05)
    canvas_nuis.SetLeftMargin(0.08)
    ...
    leg=ROOT.TLegend(0.75,0.7,0.94,0.89)
    leg.SetFillColor(0)
    leg.SetTextFont(42)
    leg.AddEntry(hist_prefit,"Prefit","FL")
    leg.AddEntry(gr_fit_comb_s,"Comb S+B fit","EPL")
    leg.AddEntry(gr_fit_sl_s,"SL S+B fit","EPL")
    leg.AddEntry(gr_fit_dl_s,"DL S+B fit","EPL")
    leg.Draw()
    fout.WriteTObject(canvas_nuis)
    if options.outputPDF:
        canvas_nuis.Print("compare_nuisance_pulls_%s.pdf" % (options.outputPDF))
        canvas_nuis.Print("compare_nuisance_pulls_%s.png" % (options.outputPDF))
